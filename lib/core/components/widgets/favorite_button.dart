...
class FavoriteButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => FavoriteButtonCubit()..checkIfFavorite(songId: songEntity.songId),
        ...
        builder: (context, state) {
          if (state is FavoriteButtonInitial) {
            return IconButton(
              ...
              icon: Icon(
                songEntity.isFavorite ? Icons.favorite : Icons.favorite_border_rounded,
                color: Colors.grey,
                size: 26,
              ),
            );
            ...
            return IconButton(
              onPressed: () {
                context.read<FavoriteButtonCubit>().toggleFavoriteStatus(songId: songEntity.songId);
              },
              icon: Icon(
